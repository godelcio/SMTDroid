...
namespace SMTDroid
{
	[Activity (Label = "People I Met")]			
	public class PeopleMet : Activity
	{
		protected override void OnCreate (Bundle bundle)
		{
			...
			SetContentView (Resource.Layout.PeopleMet);

			// Get parameter from previous activity
			string userId = Intent.GetStringExtra ("user_id") ?? "-1";

			// Get the list
			ListView peopleMet = FindViewById<ListView>(Resource.Id.listPeopleMet);
			...
			List<SMTWebsevice.Contact> wspList = _service.listContactsMet (userId).ToList ();
			foreach (SMTWebsevice.Contact item in wspList) {
				peopleList.Add (new PeopleListItem(item.id, item.name));
			}

			peopleMet.Adapter = new PeopleListAdapter(this, peopleList);

			// If any event is selected
			peopleMet.ItemClick += (object sender, AdapterView.ItemClickEventArgs e) => {
